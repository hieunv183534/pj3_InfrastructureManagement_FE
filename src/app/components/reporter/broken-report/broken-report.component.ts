...
export class BrokenReportComponent implements OnInit {
  ...
  submitForm(data: any){
    ...
    this.reportService.add(newReport).then((res: any) => {
      ...
      this.messageService.add({ key: "toastUserView", severity: 'success', summary: "SUCCESS", detail: "Báo hỏng đối tượng thành công!" });
    }).catch((err: any) => {
      this.messageService.add({ key: "toastUserView", severity: 'success', summary: "SUCCESS", detail: "Báo hỏng đối tượng thất bại!" });
    });
  }
  ...
  deleteReport(report: any){
    this.confirmationService.confirm({
      key: 'hieunvConfirm',
      header: "Xác nhận",
      message: `Bạn có thực sự muốn xóa Báo hỏng ${report.positionItem.name} này?`,
      acceptLabel: "Xóa",
      rejectLabel: "Hủy",
      accept: () => {
        this.reportService.delete(report.id).then((res: any) => {
          this.getListItem();
          this.messageService.add({ key: "toastUserView", severity: 'success', summary: "SUCCESS", detail: "Xóa báo hỏng thành công!" });
        }).catch((err: any) => {
          this.messageService.add({ key: "toastUserView", severity: 'error', summary: "FAILED", detail: "Xóa báo hỏng thất bại!" });
        });
      }
    });
  }
